...
#                print(f'player: {self.id} dead, alive_players_num: {alive_players_num}')
    ...
    def send_images(self,):
        image_accept_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        image_accept_socket.bind((SERVER, PORT+1))
        image_accept_socket.listen()
        missing_image_path = "image_path"
        while missing_image_path:
            image_socket, adress = image_accept_socket.accept()

            missing_image_path = recieve(image_socket)

            print(f'sending image: {missing_image_path}')

            if missing_image_path:
                with open(missing_image_path, mode='rb') as file:
                    image_bytes = file.read(1024)
                    while image_bytes:
                        image_socket.send(image_bytes)
                        image_bytes = file.read(1024)
            print('closing image_socket, finished image transmission')
            image_socket.close()
        print('closing image_accept_socket, finished all images transmission')
        image_accept_socket.close()
